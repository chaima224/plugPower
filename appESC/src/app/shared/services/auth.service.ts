...
export class AuthService{ 
    ...
    userLogin(userPayload: {username: string; password: string; }):Observable<boolean>{
        return this.http.post("http://localhost:8080/api/auth/login", userPayload)
            .pipe(
                map((value: any) => {
                    if (value) {
    ...
    userRegister(userPayload: {username: string; password: string;  nom: string; prenom: string ; telephone: string }):Observable<boolean>{
        return this.http.post("http://localhost:8080/api/auth/register", userPayload)
            .pipe(
                map((value: any) => {
                    if (value) {
                        localStorage.setItem("access_token", value.accessToken);
